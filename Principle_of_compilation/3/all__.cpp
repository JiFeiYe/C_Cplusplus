...
static struct
{
    const char *str;
    TokenType tok;
} reservedWords[MAXRESERVED] = {{"if", IF}, {"then", THEN}, {"else", ELSE}, {"end", END}, {"repeat", REPEAT}, {"until", UNTIL}, {"read", READ}, {"write", WRITE}, {"true", T_TRUE}, {"false", T_FALSE}, {"not", NOT}, {"and", AND}, {"or", OR}, {"int", INT}, {"string", STRING}, {"bool", BOOL}, {"do", DO}, {"while", WHILE}};
...
TreeNode *decl()
{
    TreeNode *first = type_specifier();
    first->child[0] = varlist();
    return first;
}

TreeNode *declarations()
{
    TreeNode *first = NULL, *tail = NULL;
...
{
    TreeNode *syntaxTree;
    const char *pgm = "E:\\testbianyi\\3\\tiny+.txt";
