...
private:
    ...
    CDate date;//截止日期
public:
    CD(char* name_, char type_, char media_, CDate& date__);//初始化
    CD(CD& cd_);//拷贝构造
    void testout();//输出
    ~CD();//析构
};
CD::CD(char* name_, char type_, char media_, CDate& date__)
{
    name = new char[30];
    strcpy_s(name, 30,name_);
    type = type_;
    media = media_;
    date = date__;
}
